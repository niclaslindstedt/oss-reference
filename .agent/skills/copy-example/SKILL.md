...
2. **Resolve the source path** the showcase needs. If `SOURCE_PATH` was passed, validate it; otherwise infer it from the section number.
   ...
   | §20 Source and test organization          | language-specific test trees (`tests/` for Node/Python/Rust, `*_test.go` alongside source, `src/test/` for JVM); frontend stacks also ship `playwright*.config.ts` / `vitest.config.ts` / `jest.config.*` at the repo root and per-domain subdirs under `tests/` (e.g. `visual/`, `a11y/`, `a11y-manual/`). Pair the configs with their specs and any `.claude/rules/<…>.md` files that document the workflow (e.g. visual-baseline rebaseline playbook). Exclude binary snapshot baselines from the showcase — link to the upstream path instead. |
